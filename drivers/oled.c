...
struct rt_spi_device oled_spi_device;  
...
void SPI_Config(void)
{
    ...
    result = rt_spi_bus_attach_device(&oled_spi_device,
                                      "spi11",
                                      "spi1",
                                      &spi_cs);
    if (RT_EOK != result)
    {
        rt_kprintf("[%s] attach spi device oled fail.\n", __FUNCTION__);
        return ;
    }
    ...
    rt_spi_configure(&oled_spi_device, &cfg);  
}
...
{	 		
	...
	send_buf[0] = TxData;
    rt_spi_send(&oled_spi_device, send_buf, 1);	
	delay_us(5);
}
